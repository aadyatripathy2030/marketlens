...
async function fetchLive(symbol) {
  ...
  const url = `https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(symbol)}&interval=1day&outputsize=150&apikey=${STOCK_API_KEY}`;
...
function buildDemo(symbol) {
  const closes = I.demoCloses(symbol, 140);
...
async function handleStock(req, res, symbol) {
  ...
  const sig = I.computeSignal(closes);
  const { forecast, slope } = I.linearForecast(closes.slice(-30), FORECAST_DAYS);
  ...
  json(res, 200, {
    ...
    indicators: { sma20: sig.sma20, sma50: sig.sma50, rsi: sig.rsi, trendSlope: slope },
    signal: { label: sig.label, reason: sig.reason },
    forecast,
  });
}
...
function ruleBasedSummary(p) {
  ...
  return `${p.symbol} is showing a ${p.signal.label.toLowerCase()} technical setup. ${p.signal.reason} `
    + `RSI momentum is ${rsiTxt}. A simple trend projection points slightly ${dir} over the next ${p.forecast.length} sessions. `
    + `This is a mechanical read of the price data, not a forecast you should trust for decisions — it can be wrong.`;
}
async function callClaude(p) {
  const system = 'You are a cautious market-analysis assistant. Given a stock symbol and its technical indicators, write a brief (3-4 sentence) plain-English read of the trend, momentum, and one key risk. Be balanced and explicitly note uncertainty. This is educational analysis, NOT financial advice. Do not tell the user to buy or sell.';
  const user = `SYMBOL: ${p.symbol}\nLatest: ${p.latest} ${p.currency} (${p.changePct.toFixed(2)}% vs prior day)\n`
    + `SMA20: ${p.indicators.sma20}\nSMA50: ${p.indicators.sma50}\nRSI(14): ${p.indicators.rsi}\n`
...
http.createServer(async (req, res) => {
  try {
    ...
    if (url === '/api/stock' && req.method === 'GET') {
      const sym = new URLSearchParams(req.url.split('?')[1] || '').get('symbol');
      return await handleStock(req, res, sym);
    }
